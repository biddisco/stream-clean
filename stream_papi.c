...
int main(int argc, char** argv) {
    ...
    int handle_triad = pw_new_collector("triad");
    //int handle_scale = pw_new_collector("scale");
    //int handle_copy  = pw_new_collector("copy");
    //int handle_add   = pw_new_collector("add");
    ...
    for (k=0; k<ntimes; k++)
    {
        ...
        //pw_start_collector(handle_copy);
        times[0][k] = mysecond();
            #pragma omp parallel for
            #pragma simd
        ...
        //pw_stop_collector(handle_copy);

        //pw_start_collector(handle_scale);
        times[1][k] = mysecond();
            #pragma omp parallel for
            #pragma simd
        #ifdef USE_NONTEMPORAL
        ...
        //pw_stop_collector(handle_scale);
        ...
        pw_start_collector(handle_triad);
        times[3][k] = mysecond();
            #pragma omp parallel for
            #pragma simd
        #ifdef USE_NONTEMPORAL
        ...
        pw_stop_collector(handle_triad);
    }
